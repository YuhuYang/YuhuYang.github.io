(() => {
  const dictionary = {
    ...
    about_skills_title: { zh: '技能与方法', en: 'Skills and Methods' },
    about_skills_1: { zh: '编程与分析工具：Python（熟练）、NetLogo（熟练）、R（入门）、Matlab（入门）。', en: 'Programming and analytical tools: Python (proficient), NetLogo (proficient), R (beginner), and Matlab (beginner).' },
    about_skills_2: { zh: '数据驱动语言学研究：使用 Python、R、Matlab 进行网络分析、概率统计分析等实证建模工作。', en: 'Data-driven linguistic research: using Python, R, and Matlab for network analysis, probabilistic and statistical analysis, and related empirical modeling.' },
    about_skills_3: { zh: '计算模拟语言学研究：使用 NetLogo 进行多主体建模、数学建模与语言演化模拟。', en: 'Computational simulation in linguistics: using NetLogo for agent-based modeling, mathematical modeling, and language evolution simulations.' },
    about_skills_4: { zh: '学术写作与排版：使用 LaTeX 进行论文排版与投稿材料准备。', en: 'Academic writing and typesetting: using LaTeX for manuscript formatting and submission preparation.' },
    about_skills_5: { zh: 'AI for Linguistics：使用 Codex 等 agent 辅助语言学研究，并结合多种编程语言开发语言学软件。', en: 'AI for Linguistics: using agents such as Codex to support linguistic research and developing linguistic software with multiple programming languages.' },
